...
# Em modo "congelado" (PyInstaller), __file__ aponta para dentro do bundle
# temporário; precisamos da pasta do .exe para achar configuracoes_comissoes.xlsx,
# dados_entrada/, saida/ e etc.
...
class App:
    ...
    def on_calcular(self) -> None:
        ...
        # Em modo congelado o pipeline está embutido no exe (não há .py no disco).
        if not FROZEN and not PIPELINE_PATH.exists():
            messagebox.showerror(
        ...
        # Em modo congelado, o próprio .exe age como pipeline via sentinel --run-pipeline
        if FROZEN:
            cmd = [sys.executable, "--run-pipeline", "--mes", str(mes), "--ano", str(ano)]
        else:
    ...
    def _criar_planilha_modelo(self) -> bool:
        if not FROZEN and not TEMPLATE_SCRIPT.exists():
            messagebox.showerror(
        ...
        try:
            if FROZEN:
                template_cmd = [sys.executable, "--gerar-template"]
            else:
                template_cmd = [sys.executable, str(TEMPLATE_SCRIPT)]
